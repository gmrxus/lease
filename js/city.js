...
function getContent(obj) {
	...
	for (var i = 0; i < cityArr.length; i++) {
		if (cityArr[i].name.indexOf(kw) >= 0) {
			html = html + "<div class='item' id='" + cityArr[i].cityId + "' onmouseenter='getFocus(this)' onClick='getCon(this);'>" + cityArr[i].name + "</div>"
		}
	}
	if (html != "") {
...
function getCon(obj) {
	...
	if (localStorage.getItem("action") == "quchechengshi") {
		window.localStorage.setItem("qucheCity", value);
		window.localStorage.setItem("huancheCity", value);
